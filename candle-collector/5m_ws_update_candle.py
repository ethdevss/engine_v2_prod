...
def on_message(ws, message):
    ...
    if "partial" in message:
        ...
        if str(parse_datetime_minute) == '0' or str(parse_datetime_minute) == '30':
            # 5분, 10분, 15분, 20분, 25분
            ...
            low_list.append(low)
            low_30m_price = min(low_list)
            try:
                candle_row = Candle30m.objects(timestamp=timestamp).get()
                candle_row.update(close=close, low=low_30m_price)
                print('update exist row')
            except Exception as e:
                candle30m = Candle30m(timestamp=timestamp, symbol=symbol, low=low_30m_price, close=close)
                candle30m.save()
                print(e)
